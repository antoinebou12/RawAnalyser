...
-//+++++++++++++/:::::::::::::::::::::::::::--.`                .-///////////.`            `///:             .-///////////++++`            `.///////////-.             :///`            `.:///////////-`         `:///:`          .:///-                  
  ``-:+++++++++++/::::::::::::::::::::::--.`                                                                           ``.++++`                                                                                                                           
    ...
    # raw to tiff
    tiff.imsave(str(basename(newFile)) + str(numFile) + '.tiff', matrix)
    # Dead Pixel
    ...
    def clipping():
     ...
     | |____ | || || |_) || |_) || || | | || (_| |
      \_____||_||_|| .__/ | .__/ |_||_| |_| \__, |
        ...
        print 'Clipping at 64 (16bit) for black(min) level'
        print 'B Black level ' + str(
            min(np.array(ChannelB)[np.nonzero(np.array(ChannelB))])) + ' count ' + str(
            Blue[1][1]) + ' second closest value ' + str((Blue[0][2]))
        print 'G Black level ' + str(
            min(np.array(ChannelG)[np.nonzero(np.array(ChannelG))]) ) + ' count ' + str(
            Green[1][1]) + ' second closest value ' + str((Green[0][2]))
        print 'R Black level ' + str(
            min(np.array(ChannelR)[np.nonzero(np.array(ChannelR))]) ) + ' count ' + str(
            Red[1][1]) + ' second closest value ' + str((Red[0][2]) )

        if ((min(np.array(ChannelB)[np.nonzero(np.array(ChannelB))]) <= 64) or (
                    min(np.array(ChannelG)[np.nonzero(np.array(ChannelG))]) <= 64) or (
                    min(np.array(ChannelR)[np.nonzero(np.array(ChannelR))]) <= 64)):
        ...
        print 'Clipping at 65472 (16bit) for white(max) level'
        print 'B White level ' + str(np.array(ChannelB).max() ) + ' count ' + str(
            Blue[1][(Blue[1].size - 1)]) + ' closest value ' + str((Blue[0][(Blue[1].size - 2)]) )
        print 'G White level ' + str(np.array(ChannelG).max() ) + ' count ' + str(
            Green[1][(Green[1].size - 1)]) + ' closest value ' + str((Green[0][(Green[1].size - 2)]) )
        print 'R White level ' + str(np.array(ChannelR).max()) + ' count ' + str(
            Red[1][(Red[1].size - 1)]) + ' closest value ' + str((Red[0][(Red[1].size - 2)]) )

        if ((np.array(ChannelR).max() >= 65472) or (np.array(ChannelR).max() >= 65472) or (
                    np.array(ChannelR).max() >= 65472)):
    ...
    | |__| || (_| || | | | | || | | | | || (_| |
     \_____| \__,_||_| |_| |_||_| |_| |_| \__,_|
        ...
        print (((((-np.log10(
            ((((sum(map(sum, white))) / float((np.count_nonzero(np.array(white))))) / 256) / 255))) - (-np.log10((((
                                                                                                                       (
                                                                                                                           sum(
                                                                                                                               map(
                                                                                                                                   sum,
                                                                                                                           ...
                                                                                                                           (
                                                                                                                               np.count_nonzero(
                                                                                                                                   np.array(
                                                                                                                                       neutral8))))) / 256) / 255)))) / -0.1660817663) + (
                    ((-np.log10(((((sum(map(sum, neutral8))) / float(
                        (np.count_nonzero(np.array(neutral8))))) / 256) / 255))) - (-np.log10(((((sum(
                        map(sum, neutral65))) / float(
                        (np.count_nonzero(np.array(neutral65))))) / 256) / 255)))) / -0.2056390617) + (((-np.log10(
            ((((sum(map(sum, neutral65))) / float((np.count_nonzero(np.array(neutral65))))) / 256) / 255))) - (
                                                                                                            -np.log10(
                                                                                                                ((((
                                                                                                                       sum(
                                                                                                                           map(
                                                                                                                    ...
                                                                                                                    (
                                                                                                                        np.count_nonzero(
                                                                                                                            np.array(
                                                                                                                                neutral5))))) / 256) / 255)))) / -0.2835423157) + (
                    ((-np.log10(((((sum(map(sum, neutral5))) / float(
                        (np.count_nonzero(np.array(neutral5))))) / 256) / 255))) - (-np.log10(((((sum(
                        map(sum, neutral35))) / float(
                        (np.count_nonzero(np.array(neutral35))))) / 256) / 255)))) / -0.3494155563)) / 4)
     ...
     \ \    / (_)                | | | |      
      \ \  / / _  __ _ _ __   ___| |_| |_ ___ 
       \ \/ / | |/ _` | '_ \ / _ \ __| __/ _  
        ...
        Image_arrayVig = np.reshape(np.array(vignetteMatrix), (args.width * args.height, 1))
        ...
        print (((((-np.log10(
            ((((sum(map(sum, whiteVig))) / float((np.count_nonzero(np.array(whiteVig))))) / 256) / 255))) - (
                      -np.log10(((((sum(map(sum, neutral8Vig))) / float(
                          (np.count_nonzero(np.array(neutral8Vig))))) / 256) / 255)))) / -0.1660817663) + (((-np.log10(
            ((((sum(map(sum, neutral8Vig))) / float((np.count_nonzero(np.array(neutral8Vig))))) / 256) / 255))) - (
                                                                                                                -np.log10(
                                                                                                                    ((
                                                                                                                         (
                                                                                                                             (
                                                                                                                                 ...
                                                                                                                                 (
                                                                                                                                     np.count_nonzero(
                                                                                                                                         np.array(
                                                                                                                                             neutral65Vig))))) / 256) / 255)))) / -0.2056390617) + (
                    ((-np.log10(((((sum(map(sum, neutral65Vig))) / float(
                        (np.count_nonzero(np.array(neutral65Vig))))) / 256) / 255))) - (-np.log10(((((sum(
                        map(sum, neutral5Vig))) / float(
                        (np.count_nonzero(np.array(neutral5Vig))))) / 256) / 255)))) / -0.2835423157) + (((-np.log10(
            ((((sum(map(sum, neutral5Vig))) / float((np.count_nonzero(np.array(neutral5Vig))))) / 256) / 255))) - (
                                                                                                              -np.log10(
                                                                                                                  ((((
                                                                                                                         sum(
                                                                                                                             map(
                                                                                                                      ...
                                                                                                                      (
                                                                                                                          np.count_nonzero(
                                                                                                                              np.array(
                                                                                                                                  neutral35Vig))))) / 256) / 255)))) / -0.3494155563)) / 4)

    def histogram():
        global numFile
        global newFile
        global matrix
        plt.hist(np.array(matrix.ravel()), bins=1024,
                 range=(np.array(matrix.ravel()).min(), np.array(matrix.ravel()).max()))
        plt.title('Histogram')
        fig = plt.gcf()
        plot_url = py.plot_mpl(fig, filename='HistogramRaw', height=np.array(matrix.ravel()).max())
        #Convert to plotly figure
        plotly_fig = tls.mpl_to_plotly(fig)
        py.image.save_as(plotly_fig, str(basename(newFile)) + 'histogram' + str(numFile) + '.png')
...
def _parser():
    ...
    parser.add_argument("input", default='', type=str, help='input folder to check for new raw file')
    parser.add_argument("output", default='', type=str, help='input folder to check for new raw file')
    parser.add_argument("--points", default='json/points.json', type=str,
                        help='the json file with the point for the ROI and Gray Patches ')
    parser.add_argument("--height", default=1952, type=int, help='Find the gamma or the clipping')
    parser.add_argument("--width", default=2592, type=int, help='Find the gamma or the clipping')
